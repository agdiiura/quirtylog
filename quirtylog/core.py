...
def retrieve_name(var) -> str:
    ...
    Parameters:
    - var: The input variable for which the name needs to be retrieved.
...
def path_constructor(loader, node, log_path: str | Path = 'logs') -> str:
    ...
    - node: The YAML node representing the value to be processed.
    - log_path: The base path to be used for constructing the full path (default: 'logs').

    Returns:
    - path: A valid path obtained by expanding environment variables and replacing the matched value.
...
def check_path(var: str | Path, name: str = 'value') -> Path:
    ...
    Parameters:
    - var: The input value, which can be either a string or a Path object.
    - name: The name of the input variable, used in error messages (default: 'value').

    Returns:
    - var: A Path object representing the validated input value.
...
def create_logging(log_path: str | Path = default_log_path,
    ...
    Parameters:
    - log_path: The folder path for storing log files. Defaults to `default_log_path`.
    - config_file: The configuration file path. Defaults to `default_config_file`.
    - name: The logger name. If None, it is automatically determined based on the calling module and function.
    - db: The name of the SQLite database. If provided, a SQLiteHandler will be added to the logger.
    - remove_old_log: A flag indicating whether to remove old log files from the specified log path.
...
def exception(logger: logging.Logger, level: str = 'info'):
    ...
    Parameters:
    - logger: The logging object to be used for logging exceptions.
    - level: The logging level for exception messages (default: 'info'). Possible values: {'info', 'debug', 'warning', 'error'}.
...
def clear_old_logs(log_path: str | Path = default_log_path,
    ...
    Parameters:
    - log_path: The folder path where log files are stored (default: 'logs').
    - k_min: The minimum number of log files to keep in the history (default: 1).
